...
use crate::{DatabaseKeyIndex, Event, EventKind, Id, QueryDisposition};

// ANCHOR: semantic_inspector_salsa_request_guard
...
impl RequestGuard<'_> {
    fn completed(&mut self, disposition: QueryDisposition) {
...
impl Drop for RequestGuard<'_> {
    fn drop(&mut self) {
        ...
        self.zalsa.event(&|| {
            Event::new(EventKind::DidRequest {
                database_key,
                disposition,
            })
        });
    ...
    C: Configuration,
{
    ...
    pub fn fetch<'db>(
        ...
        let (memo, _) = self.refresh_memo_traced(db, zalsa, zalsa_local, id);
    ...
    // ANCHOR_END: semantic_inspector_salsa_request_guard

    /// Refresh a memo under the complete request lifecycle. Both ordinary
    /// tracked-function fetches and accumulator graph traversal use this path,
    /// so neither can execute or reuse a tracked function invisibly.
    #[inline(always)]
    pub(super) fn refresh_memo_traced<'db>(
        &'db self,
        db: &'db C::DbView,
        zalsa: &'db Zalsa,
        zalsa_local: &'db ZalsaLocal,
        id: Id,
    ) -> (&'db Memo<'db, C>, QueryDisposition) {
        let database_key_index = self.database_key_index(id);
        let _span =
            crate::tracing::info_span!("salsa_query", database_key = ?database_key_index).entered();
        zalsa.event(&|| {
            Event::new(EventKind::WillRequest {
                database_key: database_key_index,
            })
        });
        let mut request = RequestGuard {
            zalsa,
            database_key: database_key_index,
            disposition: QueryDisposition::Cancelled,
        };

        zalsa.unwind_if_revision_cancelled(zalsa_local);

        let result = self.refresh_memo(db, zalsa, zalsa_local, id);
        request.completed(result.1);
        result
    }
    ...
    ) -> Option<(&'db Memo<'db, C>, QueryDisposition)> {
        let memo = self.get_memo_from_table_for(zalsa, id, memo_ingredient_index)?;

        memo.value.as_ref()?;

        let database_key_index = self.database_key_index(id);
        let was_current = memo.verified_at.load() == zalsa.current_revision();
        ...
        if can_shallow_update.yes() && !memo.may_be_provisional() {
            ...
            unsafe {
                Some((
                    ...
                    if was_current {
                        QueryDisposition::Reused
                    } else {
                        QueryDisposition::Validated
                    },
                ))
            }
        } else {
    ...
    fn fetch_cold<'db>(
        ...
        if let Some(old_memo) = opt_old_memo {
            if old_memo.value.is_some() {
                    ...
                    )
                {
                    ...
                    return unsafe {
                        Some((
                            self.extend_memo_lifetime(old_memo),
                            QueryDisposition::Validated,
                        ))
                    };
                }
        ...
        self.execute(db, claim_guard, opt_old_memo)
            .map(|memo| (memo, QueryDisposition::Executed))
    }
            ...
            CycleRecoveryStrategy::Fixpoint | CycleRecoveryStrategy::FallbackImmediate => {
                ...
                if let Some(memo) = &memo_guard {
                        ...
                        && memo.revisions.cycle_heads().contains(&database_key_index)
                    {
                        ...
                        return (
                            unsafe { self.extend_memo_lifetime(memo) },
                            QueryDisposition::Reused,
                        );
                    }
                }
