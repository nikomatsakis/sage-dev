...
crates/sage-ir/src/check/
  ...
  solve/
    ...
    boundary.rs     proof-local state plus query/response instantiation
    prove.rs        structural proving, active proof frames, and candidates
    clauses.rs      environment and local-impl candidate assembly
    merge.rs        order-independent reduction and directional subsumption
...
- [x] Freeze the MVP atom language to types only:
  - [x] `TraitImpl { self_ty: Ptr<Ty>, trait_ref: TraitRef }`
  - [x] `Equals(Ptr<Ty>, Ptr<Ty>)`
- [x] Freeze the MVP structural goal language to `Atom`, `All`, `Exists`,
  `Implies`, and `Maybe`.
- [x] Explicitly defer `ForAll`, normalization/projections, outlives proving,
  higher-ranked lifetimes, auto-trait structural candidates, external impls,
  coherence, specialization, and negative reasoning.
- [x] Land the [Trait System](../trait-system/README.md) data model needed by the
...
- [x] Ensure an impl's generic parameters bind its self type, trait reference,
  and where-clauses as one coherent scope. The solver must be able to open all
  three with one fresh candidate substitution.
- [x] Land the Trait System's deterministic
  `local_impls(db, LocalCrateSymbol)` query. The solver linearly scans that
  list; indexing is deferred.
- [x] Use `LocalCrateSymbol` as explicit query context. Candidate discovery must
  never infer the local crate from ambient state.
- [x] Set `MAX_PROOF_DEPTH` to a named MVP default of 64. Reaching the limit
  yields `Maybe`, while an inductive ancestor cycle yields `No`.
- [x] Reconcile the trait-solving README and implementation sketches with these
  frozen types before using them as coding references.

Readiness tests and fixtures:

- [x] An impl fixture can open one binder and observe the same generic identity
  in its self type, trait reference, and where-clause body.
- [x] Two local crates containing different impl sets enumerate different
  candidates for the same trait atom.
- [x] No MVP Rust enum contains a placeholder variant for a deferred feature.
...
Candidate alternatives use independent proof contexts. Each keeps its local
child transaction isolated, extracts a response, and drops the context. They
never collapse state into a requester or another candidate.

- [x] Make all egraph reads and writes take an explicit `Version`, or operate
  through an `EGraphView`/branch handle that permanently names one. Remove
  `set_current_version` from solver-facing code.
- [x] Make inference-variable IDs globally unique across sibling versions and
  record each variable's owner version. Sibling allocation must not reuse the
  same `InferVarIndex`.
- [x] Keep version ownership generation-safe: either do not reuse a `Version`
...
- [x] Enforce visibility: a version sees ancestor variables plus its own, but
  never variables owned by a sibling or descendant.
- [x] Enforce leaf-only per-version mutation. Creating any child freezes the
...
- [x] Add explicit `branch_from(parent)`, `collapse_into(child, parent)`, and
...
- [x] Add a child-probe helper that cannot return without committing or
  discarding its child.
- [x] Implement private in-probe structural unification over `Ty` using the
  egraph and `skeleton::{decompose, recompose}`.
- [x] Expose `try_unify(parent, lhs, rhs)` as the transactional,
  diagnostic-free API. Do not expose a partially mutating public variant.
- [x] Make a batch of structural equalities one transaction, not one child per
  equality. Step 4 uses this helper for `Subst` application after the solver IR
  exists.
- [x] Perform an occurs check before commit, including indirect cycles after
  following egraph representatives. Reject `?X = Vec<?X>` and mutually
  recursive forms.
- [x] Perform universe/leak validation before commit. A variable may not be
...
- [x] Store each variable's immutable creation universe separately from a
...
- [x] Treat a committed ceiling decrease as a semantic update: advance the
  affected variable/dependency revision and publish its wake exactly once.
  Discarded lowering publishes neither.
- [x] Reuse the same accessibility validator for versioned `Bound` writes.
...
- [x] Make collapse re-own every committed child variable before invalidating
  the child handle, preserving its globally unique ID and universe.
- [x] Stage rebuild worklists, dependent updates, bound changes, and variable
  wakes in the probe. Publish them exactly once on commit and never on discard.
- [x] Route `InferCtx::require_eq` through `try_unify` and preserve its current
  diagnostics and `Ty::Error` behavior.
- [x] Keep alias expansion and associated-type normalization out of this helper.

Tests:

- [x] Equal and unequal concrete leaves.
- [x] Inference-variable binding and coalescing.
- [x] Nested structural unification and skeleton/arity mismatches.
- [ ] Congruence after rebuild.
- [x] A late mismatch rolls back earlier child equalities from the same
  recursive unification.
- [x] A failed batch of equalities rolls back every earlier entry.
- [x] Direct and indirect occurs-check failures leave the parent unchanged.
- [x] Universe leaks and child-local-variable escapes are rejected.
...
- [x] Add a scoped runtime API whose child futures may borrow the proof context
  and whose scope cannot finish until every child is joined or cancelled.
- [x] Replace no-op polling wakers with real task wakers whose `wake` and
...
- [x] Make cancellation explicit. Dropping sibling futures after an
...
- [x] Do not discard or collapse a branch while a task, waiter, or descendant
  branch still references it.
- [x] Provide a completion stream for scoped candidate tasks, but do not make
  scheduler arrival order part of answer identity.

Tests:

- [x] Sibling branches allocate distinct variables and cannot inspect each
  other's variables or diffs.
- [x] A semantic write to a parent with one or more live descendants is rejected
...
- [x] A normal pending future registers its runtime waker, is polled again after
...
- [x] Cancelling a pending branch discards its egraph state and registrations
  and wakes/finishes the join scope.
- [ ] Cancelling an early, middle, or late sibling produces identical surviving
  parent state.
- [x] A scope cannot return while a child still owns a branch.
...
- [x] Define `GoalQueryData` with exactly these contextual fields:
  ...
  than the cached key.
- [x] Define canonical variable metadata as
  `{ param, kind, role, relative_universe }`, where `role` is either
  `RigidPlaceholder` or `ExistentialInput`.
- [x] Define the type-only `Atom` variants frozen in Step 0. Keep `self_ty`
  explicit instead of hiding it in trait arguments.
- [x] Define MVP `Goal`, `Assumption`, binder-owned `Clause`, `ClauseSource`,
...
- [x] Make assumption heads statically trait-only. `Equals` is an intrinsic
...
- [x] Give every clause one binder covering its head and body. Opening a clause
  must produce one fresh mapping shared by both.
- [x] Restrict `Subst` keys to canonical variables with role
  `ExistentialInput` and kind `Type`; reject duplicate keys deterministically.
- [x] Give response-bound existentials explicit kind and relative-universe
  metadata rather than storing an untyped count.
- [x] Define constructors that flatten nested conjunctions, preserve binder
  scope, and use `All([])` as the canonical true residual.
- [x] Restrict MVP `Exists` binders to type parameters and keep their
  alpha-equivalent IDs in the query parameter space distinct from free inputs
  and response-bound variables.
- [x] Keep trait-atom dispatch separate from equality-atom dispatch; generic
  orchestration must not assume every atom has a trait symbol or self type.

Tests:

- [x] Empty and nested conjunction normalization.
- [x] Clause head/body references remain in the same binder scope.
- [x] Duplicate or rigid substitution keys are rejected.
- [x] Response binders cover `subst` and `modulo` together.
- [x] Structurally identical goal data with different `local_crate` values is
...
- [x] Deferred atom and goal forms cannot be constructed through the MVP IR.
- [x] An equality assumption cannot be constructed; `Implies` with a trait fact
...
- [x] Canonicalize the goal and assumptions in one deterministic traversal.
- [x] Carry the source `CheckedParameterEnv` completeness/eligibility bit into
  `GoalQueryData::assumptions_complete`; never lose it while copying only the
  represented positive assumptions.
- [x] Choose and retain an absolute caller universe base. Record the caller's
...
- [x] Canonicalize nested `Exists` binders with a push/pop binder map. Rename
...
- [x] Preserve kind and universe as relative canonical metadata and retain the
  reverse mapping to the caller's original variables/placeholders.
- [x] For every existential input, canonicalize its current versioned universe
...
- [x] Instantiate rigid canonical variables as non-bindable placeholders and
  existential inputs as fresh inference variables in the query egraph.
- [x] Import the query at `canonical_universe`; when applying a response, add
  each relative universe to the absolute base retained by that caller mapping.
- [x] Extract substitutions only for existential inputs whose relation to the
  input changed.
- [x] Project egraph classes to a deterministic response normal form before
  ...
  orientation.
- [x] Eliminate proof-local variables through query inputs when possible and
...
- [x] Canonicalize every definite response with one memo table from fork-local
  variables to response-bound existentials. Reuse that table across `subst`
  and `modulo` so repeated variables remain repeated.
- [x] Preserve each response existential's relative universe and reject a
  response that would leak a local variable or placeholder into an inaccessible
  caller universe.
- [x] Use each variable's lowered current universe ceiling, not immutable
...
- [x] Validate the extracted boundary object contains no raw egraph
  `InferVarIndex`, version handle, or uncanonicalized binder reference before a
  candidate/frame version can be discarded.
- [x] Open a response binder with a fresh mapping on every instantiation, then
  apply the entire response substitution to the caller in one child probe.
- [x] Instantiate residual goals with a separate capture-avoiding push/pop
...
- [x] Reserve shallow, non-repeating freshening for ambiguous anti-unification
...
- [ ] An eclass containing flexible input `?X` and rigid placeholder `P`
  ...
  to bind or erase `P`.
- [x] Rigid and flexible variables with the same kind and universe do not share
  a canonical identity.
- [ ] Assumptions participate in canonicalization and retain flexible inputs.
- [x] Relative universes round-trip and higher-universe placeholders do not
...
- [x] Failed response application rolls back all bindings and wake effects.
- [x] Discarding the source branch after extraction leaves the response fully
...
- [x] Implement type-only anti-unification independently of the accumulator.
...
- [x] Carry each input's response binder through hint accumulation.
...
- [x] Give every shallow divergent witness the relative universe of its
...
- [x] Normalize the merged hint: remove a substitution entry whose entire
...
- [x] Implement `A.subsumes(B)` directionally: assume/freeze `B` (the
  antecedent), instantiate only `A`'s response-bound existentials as flexible
  witnesses, and check that `B` implies `A`.
- [x] Never use symmetric unification that can bind antecedent variables.
- [x] Preserve response-variable universe ceilings during comparison.
...
- [x] Compare substitutions structurally, then compare canonical flattened
  residual conjunctions by subset for the conservative MVP implication check.
- [x] Treat cases requiring trait reasoning as non-subsuming. A false negative
  may produce `Maybe`; a false positive could discard a valid alternative and
  is not allowed.
- [x] Implement complete-set reduction and the finalization rules above.

Tests:

- [x] All alternatives returning `Maybe` produce `Maybe`, including when every
  hint is empty.
- [x] `Maybe` arriving before an unconditional `Yes` does not terminate search
...
- [x] Every permutation of the same `No`/`Maybe`/`Yes` multiset produces the
  same alpha-canonical result.
- [x] All `No` produces `No`; a lone conditional `Yes` remains `Yes`; a
...
- [x] `?X = u32` and `?X = i32` become `Maybe` with an empty common hint; the
  anti-unified bare fresh witness is recognized as tautological and pruned.
- [x] Shared constructors are retained and differing tuple components receive
...
- [x] Unconditional answers subsume conditional answers; residual subset
  subsumes residual superset; binder alpha-renaming does not change the result.
- [x] An unconstrained `X` subsumes `X = u32`, while `X = u32` does not subsume
  unconstrained `X`. The latter check must not bind/freeze the antecedent into
  making the implication appear true.
- [x] Definite-answer repeated variables remain shared during subsumption;
...
- [x] Create exactly one whiteboard per actual execution of the tracked
...
- [x] Give every frame producer a fresh, isolated proof stash and egraph, and
  give every candidate an independent proof context with a local child
  transaction. A live producer therefore never borrows or descends from the
  request version on which its response will be applied (D8).
- [x] Define `AtomKey` from the canonical atom, canonical environment,
  `local_crate`, and remaining depth. Key an entry by `(AtomKey, parent_frame)`.
- [x] Deduplicate an in-progress request only when the full entry key, including
  parent frame, matches.
- [x] Before insertion, walk the parent chain and compare canonical atom,
  environment, and crate while deliberately ignoring depth. A match is an
  inductive cycle and returns `No`.
- [x] Return `Maybe` before spawning when the depth limit is exhausted.
- [x] Give each frame a stable ID, explicit pending/ready/abandoned state, one
  write-once stashed result, subscriptions, and a producer handle. Cancellation
  removes the key, marks the tombstoned frame abandoned, and synchronously
  drops the producer outside the whiteboard borrow.
- [x] Allocate one subscription when each `ProofFuture` is created, update that
...
- [x] Start a new frame only in the query-owned producer scope. Import the
  canonical `AtomKey` into a fresh producer-owned proof context; do not clone
  or inherit the creating requester's candidate variables or version.
- [x] Keep the producer alive when its creator is cancelled but another
  subscription remains. The producer handle belongs to the frame/query scope,
  not to any one requester.
- [x] When the last subscription to an incomplete frame disappears, remove its
  key before requesting cancellation so a later lookup can create fresh work.
  Mark the old ID as an abandoned tombstone, take and drop the producer outside
  the whiteboard borrow, and let scoped-task drop recursively cancel nested
  work and remove its subscriptions before dropping each proof context.
- [x] On normal completion, stash a branch-independent response, join every
  nested task, and drop the producer proof context before storing the ready
  result and waking subscribers. Completed entries remain reusable until query
  teardown.
- [x] Make the query scope join or cancel-and-drain every producer before the
...
- [x] Reject double completion and stale frame access with assertions in debug
  builds.

Tests:

- [x] Same-parent duplicates share a frame; different-parent requests do not.
...
- [x] Direct and indirect ancestor recursion are rejected even when the repeated
  request has a different depth value.
- [ ] Equal completed non-ancestor frames do not count as cycles.
- [x] Depth exhaustion returns `Maybe` without allocating a frame.
- [x] Multiple subscribers transition from pending to the same completed result.
- [x] Cancelling the candidate which created a frame unregisters only its
...
- [x] Cancelling one non-creator subscriber leaves the independently needed
  producer running.
- [x] Cancelling all subscribers removes the key, cancels and joins the
...
- [x] Success and cancellation both stop every task which can access the frame
  version before discarding it; success publishes/wakes only after that discard.
- [x] Cancelling the producing query scope resolves or removes every owned
  entry and leaves no task or branch behind.
- [x] Frame IDs and results remain stable as later frames are appended.
...
- [x] Prove `Equals` directly with transactional `try_unify`; do not send it
  through trait candidate assembly.
- [x] Implement `All([])`, `All`, `Exists`, `Implies`, and `Maybe`.
- [x] Give a conjunction its own child version. Collapse it into its caller for
  a possible/successful result and discard it on `No`, so a failed conjunction
  cannot leak earlier sibling equalities.
- [x] Make cancellation of a conjunction cancel and join its nested scoped work
...
- [x] Open `Exists` variables as fresh local existentials with explicit
  universe metadata.
- [x] Extend the environment only while evaluating `Implies`. If the inner
  proof returns a nontrivial residual `R`, return
  `Implies(original_assumptions, R)`; do not return bare `R` to an outer scope.
- [x] Canonicalize every atomic attempt through the current egraph state. Track
  each pending goal's last attempted semantic revision so unchanged state does
  not re-run it.
- [x] Replace a conjunct with a returned residual and immediately mark that
...
- [x] Retry stalled goals only after their normalized form or a relevant
  inference revision changes. At fixpoint, return the canonical conjunction of
  remaining residuals.
- [x] Keep a `Maybe` conjunct pending at its current substituted/canonicalized
  caller state. At the public boundary, `Maybe` carries only hard hints and the
  obligation manager retains the original goal for retry, as defined by the
  result contract.
...
- [x] A sibling equality pins a variable and causes a previously stalled atom
...
- [ ] Equivalent residuals with different nesting/order normalize to the same
  progress key.
- [x] An `Implies` assumption does not leak to a sibling.
...
- [x] Build the free-function body solver environment from its opened parameter
  environment and preserve eligibility in `assumptions_complete`. Selected
  trait/impl methods will add their opened owner environment through the same
  staging API when Method Resolution supplies the selected method; method
  ownership remains outside this RFD.
- [x] Include the Trait System's deduplicated elaboration of eligible local
...
- [x] Lower direct and implication assumptions into binder-owned clauses.
- [x] Short-circuit a `TraitImpl` atom containing `Ty::Error` to recovery
...
- [x] Assemble matching environment clauses before considering impl clauses,
  including when the self type is a bare flexible variable.
- [x] Open each candidate binder once in a fresh isolated egraph branch; use the
  same mapping for its head and body.
- [x] Match the head transactionally, prove the body as a conjunction in that
  branch, extract one canonical response, and then discard the branch.
- [x] Launch alternatives through a scoped runtime, feed completed responses to
  the Step 5 accumulator, and join every task before returning.
- [x] On an unconditional winner, explicitly cancel and clean up siblings
  before returning the result.
- [x] Submit nested trait atoms through the Step 6 whiteboard.
- [x] After environment assembly, treat a still-unproven bare flexible self
...
- [x] `T: LocalTrait<U>` exposes the local trait's instantiated defining
...
- [ ] Interleaved candidate completion and every completion order produce the
  same answer.
- [x] An unconditional environment candidate cancels pending siblings without
  leaking their bindings, waits, or wakes.
- [x] A direct `?X: Trait` environment assumption proves the identical goal
...
- [x] Enumerate local trait impls using `GoalQueryData.local_crate`; do not scan
  another crate or use an ambient source root.
- [x] Start with a deterministic linear scan of the Trait System's
  ...
  query.
- [x] Convert each `ImplSignature` into one clause whose binder covers the impl
  generics in the self type, trait reference, and where-clause body.
- [x] Expose that clause only when both the impl signature and referenced local
...
- [x] Open impl generics freshly for every candidate attempt and convert trait
  where-predicates into body goals.
- [x] Add the referenced local trait signature's instantiated where-predicates
...
- [x] Defer a local impl of an external trait until checked metadata exposes
  that trait's defining predicates; never assume the missing predicate set is
  empty.
- [x] Preserve environment-first candidate assembly. Final meaning remains
  independent of enumeration and completion order.
- [x] Keep external impl discovery, auto-trait candidates, specialization, and
  coherence out of this step.
- [x] Permit an external trait goal to succeed from an environment assumption,
...
- [x] Concrete and generic impl proofs.
...
- [x] The same canonical atom queried with two `LocalCrateSymbol`s sees only the
...
- [x] Define an interned `GoalQuery` wrapping stashed `GoalQueryData`. Because
  that data contains `local_crate`, structurally identical goals in different
  crates must not share a query.
- [x] Add the tracked synchronous `GoalQuery::prove` boundary returning a
  `Stashed<QueryResult>`.
- [x] Validate that public query inputs are canonical and contain no caller
  egraph IDs or noncanonical binder references.
- [x] For each execution, create a root proof context, result stash,
  explicit-version egraph, whiteboard, and runtime scope.
- [x] Run proof search to completion, extract the response with sharing and
  universes preserved, and verify all child scopes/branches are gone before
  stashing the result.
- [x] Provide caller helpers that canonicalize a goal and retain its reverse
...
- [x] Rigid-versus-flexible inputs and relative universes survive the complete
  caller -> query -> caller round trip.
- [x] Repeated candidate variables remain shared in substitutions and
  residuals after stashing.
- [x] Direct/indirect cycles and the depth cap produce the specified `No` and
  `Maybe` outcomes.
- [x] Query crate context controls local impl discovery and salsa identity.
- [x] Early cancellation leaves the root egraph and runtime quiescent.
...
- [x] Add an obligation record containing the canonical goal and environment,
...
- [x] Expose ordinary and transaction-local obligation submission APIs. Lower
...
- [x] Wire the body checker's ordinary free-function calls and ADT
...
- [x] Deduplicate obligations by canonical goal, environment, crate, and caller
  mapping while retaining all diagnostic provenance that matters.
- [x] On `Yes`, apply the substitution atomically and enqueue nontrivial
...
- [x] On `Maybe`, apply only its necessary hints in one transaction, retain the
...
- [x] For the MVP, derive the conservative stalled-on set from every
...
- [x] On `No`, record a trait-obligation failure at the original provenance.
- [x] Recanonicalize and retry an obligation only when a relevant variable or
  egraph revision changes. Repeated processing without a change must be
  idempotent.
- [x] Replace one-shot `block_on`/finalize ordering with a body-completion state
...
- [x] At stable quiescence, finalize unresolved variables to their final bound
...
- [x] When all variables are final but stable obligations or lookup waits
  ...
  terminal.
- [x] At final fixpoint, turn every remaining `Maybe` or nontrivial residual
...
- [x] A deferred obligation converges after later type information and updates
  the caller egraph.
- [x] A generic free-function call and generic ADT use each submit their
...
- [x] Two equivalent submissions deduplicate while diagnostics retain useful
  provenance.
- [ ] Unrelated inference changes do not re-run a stalled obligation; a
  relevant wake does.
- [x] Re-proving without an egraph change is idempotent and terminates.
...
- [x] Conflicting `Yes` substitution or `Maybe` hint application rolls back.
- [x] Finalization discharges newly enabled residuals before diagnosing the
  remainder.
...
- [x] Permanently ambiguous, depth-limited, and unprovable obligations produce
  diagnostics rather than silently completing body checking.
- [x] Finalization leaves no live solver task, whiteboard waiter, branch, or
...
- [x] Tick landed steps in this file as they land.
- [x] Keep the trait-solving README and both implementation sketches consistent
  with the implemented IR, version lifecycle, whiteboard key, answer algebra,
  and obligation lifecycle.
- [x] Update the relevant pages under `md/design/` when the solver, runtime, or
  checking pipeline becomes part of the intended architecture.
- [x] Add or revise a decision in `md/design/decisions.md` if implementation
  changes a cross-cutting, load-bearing decision.
- [x] Update RFD status/navigation and `md/implementation/roadmap.md` only at
  the lifecycle transitions required by
  [Maintaining This Book](../../contributing/maintaining-the-docs.md).
- [x] Run `cargo fmt` after every Rust edit.
- [x] Run focused tests for each step, then the full relevant test suite and
  `cargo build`.
- [x] Run `mdbook build` with link checking available and inspect the rendered
  trait-solving pages and internal links.
- [x] Confirm the final implementation has tests for rollback, occurs/leak
