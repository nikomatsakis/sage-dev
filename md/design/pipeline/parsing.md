...
- **Per-item CST and later parameter identity.** The [struct-signature
...
- Tree-sitter recovery is not yet exposed as a structured phase-completeness
  result or a complete diagnostic set.
- Local symbol stability is tested for selected offset changes and owners, not
  arbitrary insertion, deletion, and reordering for every item kind.
